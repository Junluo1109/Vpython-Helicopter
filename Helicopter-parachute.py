...
for i in range(N):
    if i == (N - 1):
        ball.append(box(size = vector(4, 4, 4), pos=vec(0, -i*d, 0), v=vec(0,0,0), texture = 'https://i.imgur.com/MwNNoyN.png'))
    else:    
...
def DragForce(v, C, A): # 空氣阻力：1/2*空氣密度*球速度^2*球體阻力係數*求迎風面積
...
while True:
    ...
    for i in range(N):      
        ...
        if i == 0: # 連接直升機那顆球速度跟直升機一樣
        ...
        elif i == N-1:  # 降落傘(僅連接一顆彈簧)
            F_g = m*g
            F_t = SpringForce(spring[i - 1].axis)
            F_drag = -DragForce(ball[i].v, Cp, A2)
            ...
            F_drag = -DragForce(ball[i].v, Cd, A)
            F_total = F_g + F_t + F_drag
            ball[i].v += (F_total/m)*dt
